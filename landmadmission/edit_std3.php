...
//$id=$_GET['id'];
$id=$_GET['id'];
$result2 = mysqli_query($conn, "SELECT * FROM stdregistration3 WHERE stdid='$id' AND payment='PAID'");
		...
		
		{
			...
			echo "<script>alert('Applicant Paid!. It cannot be edited pls contact the Director.'); window.location='stdreg3.php';</script>";
...
?>
  ...
  <?php
	include('../connect.php');
	$id=$_GET['id'];
	$result = mysqli_query($conn, "SELECT * FROM stdregistration3 where stdid='$id'");
...
?>
<form id="drop_list" name= "drop_list" action="execeditstd3.php" method="post" enctype="multipart/form-data" onSubmit="return validateForm()">
