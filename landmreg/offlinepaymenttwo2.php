...
?>
 ...
 include('../connect.php');                
		...
		if ($fremaining > $price) {
			...
			echo "<script> alert('THE PART PAYMENT IS MORE THAN THE REMAINING BALANCE!'); window.location.replace('mpayment2.php')</script>";
			
		}
		elseif ($price==$fremaining) {
			
			
        	$result3=mysqli_query($conn, "UPDATE stdregistration2 SET payment ='$payment', payment_method ='$paymentmethod', part ='$partpay', fpart ='$fremaining', total ='$remaining', Date_paid ='$datecreated' WHERE stdid ='$IDS'");
			
			$update=mysqli_query($conn, "INSERT INTO stdregistrationtrack2 (sname, firstname, oname, admno, yearofadmission, term, class, section, price, payment, payment_method, part, fpart, total, Date_paid)
VALUES
('$fname', '$firstname', '$oname', '$admno', '$yearofadmission', '$term', '$class', '$section', '$price', '$payment', '$paymentmethod', '$partpay', '$fremaining', '$remaining', '$datecreated')");
        	...
        	$result3=mysqli_query($conn, "UPDATE stdregistration2 SET payment ='PARTPAID', payment_method ='$paymentmethod', part ='$partpay', fpart ='$fremaining', total ='$remaining', Date_paid ='$datecreated' WHERE stdid ='$IDS'");
