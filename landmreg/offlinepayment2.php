...
?>
        ...
        <tr bgcolor="#999999">
          <td colspan="4" align="center"><div class="text-center">
								<button class="btn btn-dark-green" type = "submit" value = "submit" name = "update" id="update">Save</button>
								</div></td>
        </tr>
      </table></td>
    </tr>
 ...
 include('../connect.php');                
		...
		if ($fremaining > $price) {
			...
			echo "<script> alert('THE PART PAYMENT IS MORE THAN THE REMAINING BALANCE!'); window.location.replace('mpayment.php')</script>";
			
		}
		elseif ($price==$fremaining) {
			
			
        	$result3=mysqli_query($conn, "UPDATE stdregistration SET payment ='$payment', payment_method ='$paymentmethod', part ='$partpay', fpart ='$fremaining', total ='$remaining', Date_paid ='$datecreated' WHERE stdid ='$IDS'");
			
			$update=mysqli_query($conn, "INSERT INTO stdregistrationtrack (sname, firstname, oname, admno, yearofadmission, term, class, section, price, payment, payment_method, part, fpart, total, Date_paid)
VALUES
('$fname', '$firstname', '$oname', '$admno', '$yearofadmission', '$term', '$class', '$section', '$price', '$payment', '$paymentmethod', '$partpay', '$fremaining', '$remaining', '$datecreated')");
        	...
        	$result3=mysqli_query($conn, "UPDATE stdregistration SET payment ='PARTPAID', payment_method ='$paymentmethod', part ='$partpay', fpart ='$fremaining', total ='$remaining', Date_paid ='$datecreated' WHERE stdid ='$IDS'");
